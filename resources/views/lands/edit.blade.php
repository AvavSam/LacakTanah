...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <form action="{{ route('lands.update', $land) }}" method="POST" enctype="multipart/form-data" class="bg-white shadow rounded-lg p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="owner_name" id="owner_name" value="{{ old('owner_name', $land->owner_name) }}"
        ...
        <div>
            ...
            <input type="text" name="kode_bidang" id="kode_bidang" value="{{ old('kode_bidang', $land->kode_bidang) }}"
                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
            @error('kode_bidang')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <textarea name="alamat" id="alamat" rows="2"
                      class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                      placeholder="Desa, Kecamatan, Kabupaten...">{{ old('alamat', $land->alamat) }}</textarea>
            @error('alamat')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="text" name="kecamatan" id="kecamatan" value="{{ old('kecamatan', $land->kecamatan) }}"
                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
            @error('kecamatan')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <input type="number" name="luas_m2" id="luas_m2" value="{{ old('luas_m2', $land->luas_m2) }}"
                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                   min="0">
            @error('luas_m2')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        ...
        <div>
            ...
            <div class="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="latitude" id="latitude" value="{{ old('latitude', $land->latitude) }}"
                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                           readonly>
                    @error('latitude')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="text" name="longitude" id="longitude" value="{{ old('longitude', $land->longitude) }}"
                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                           readonly>
                    @error('longitude')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        ...
        <div>
            <label for="dokumen" class="block text-sm font-medium text-gray-700">Upload Dokumen Baru <span class="text-sm text-gray-500">(opsional)</span></label>
            ...
            @if($land->dokumen_path)
                <p class="mt-1 text-sm text-gray-600">Dokumen saat ini:
                    <a href="{{ asset('storage/' . $land->dokumen_path) }}" target="_blank" class="text-blue-600 hover:underline">
        ...
        <div>
            ...
            <input type="date" name="dokumen_expiry" id="dokumen_expiry" value="{{ old('dokumen_expiry', $land->dokumen_expiry?->format('Y-m-d')) }}"
                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
        ...
        <div>
            <label for="photo" class="block text-sm font-medium text-gray-700">Upload Foto Batas Lahan Baru <span class="text-sm text-gray-500">(opsional)</span></label>
            ...
            @if($land->photo_path)
                ...
                <img src="{{ asset('storage/' . $land->photo_path) }}" alt="Foto Lahan" class="mt-1 w-full max-w-xs rounded-md shadow-sm">
            @endif
        </div>
        ...
        <div class="flex justify-end space-x-2">
            ...
            <button type="submit" class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-md">Perbarui</button>
        </div>
    </form>
</div>
@endsection
